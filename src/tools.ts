...
Limitations: specs are compiled from manufacturer and carrier-published sources with ≥7 cited sources per record. Provenance is PENDING, not verified — read the envelope's provenance_status rather than this sentence. Second-agent coverage is PARTIAL: 77 citations were read and verdicted by a second agent on 2026-08-03, then a re-sourcing pass the same day added 71 further citations that carry first-agent verdicts only and have not yet been second-agent read. Provenance stays pending until that second pass completes. Treat tare_weight with particular caution: it is still confirmed by no non-tertiary source on 11 of the 16 records — including PGA, whose tare rests on carrier pages that no automated fetch has been able to reach. Confidence is medium and a PROVENANCE_PENDING advisory rides every response. Pallet records (PMC, PAG, PGA, PLA, PAJ, PMCQ7) have NO internal dimensions — a pallet has no walls or roof; read max_build_up_height_cm for the aircraft contour ceiling and do not multiply dimensions to get a volume. Airline-specific ULD variants still differ; confirm operationally critical dimensions with the carrier.
