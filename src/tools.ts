...
Limitations: specs are compiled from manufacturer and carrier-published sources with ≥7 cited sources per record. Provenance is PENDING, not verified — read the envelope's provenance_status rather than this sentence. Second-agent coverage is PARTIAL: 108 of 148 citations have been read and verdicted by a second agent, 40 still carry first-agent verdicts only. Provenance stays pending until that pass completes. The second agent has also WITHDRAWN stamps — timestamped reads fell from 93 to 74 when pages a first pass had confirmed turned out not to list the code, or to disagree once read — so treat an unstamped citation as evidence that was tried and failed, not evidence not yet gathered. Treat tare_weight with particular caution: it is confirmed by no non-tertiary source on 11 of the 16 records. PGA's 565 kg is now read-confirmed by one carrier (Cathay, basis stated on the page as including nets) while two other carrier pages publish 535 kg and 505-545 kg on that same basis, so it is the top of a contested band rather than an agreed figure. Confidence is medium and a PROVENANCE_PENDING advisory rides every response. Pallet records (PMC, PAG, PGA, PLA, PAJ, PMCQ7) have NO internal dimensions — a pallet has no walls or roof; read max_build_up_height_cm for the aircraft contour ceiling and do not multiply dimensions to get a volume. Airline-specific ULD variants still differ; confirm operationally critical dimensions with the carrier.
